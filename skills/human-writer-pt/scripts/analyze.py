#!/usr/bin/env python3
"""
human-writer-pt / analyze.py — deterministic AI-tell detector (Brazilian Portuguese).

Specialization of the master human-writer analyzer for pt-BR. Differences vs
the master (everything else is identical):
  - --lang choices=["pt"].
  - detect_tricolons uses the Portuguese conjunction (e).
  - NEW detect_accent_stripping: flags prose whose accented-character ratio is
    far below the native Portuguese floor (an entire file emitted without
    ç ã õ á é í ó ú â ê reads as broken Portuguese — a frequent AI/plain-text
    failure mode). Wired as a low-weight branch; guarded against false positives
    on very short input. Ported from the master French accent hard-check.
...
def count_sentences(text: str) -> int:
    # Portuguese sentence terminators are [.!?] — same as the master. The
...
def build_recommendations(tells: dict) -> list[dict]:
    ...
    if acc and acc.get("flag"):
        recs.append({
            ...
            "action": (
                f"Restore Portuguese accents: accented-char ratio is {acc.get('value')} "
                f"(floor {acc.get('threshold')}). De-accented prose (anos→anos for 'anos', "
                "informacao for 'informação', voce for 'você') reads as broken Portuguese. "
                "Re-accentuate ç ã õ á é í ó ô ú â ê before publishing."
            ),
        })
...
def compute_score(tells: dict, weights: dict) -> int:
    ...
    if tricolons >= 3:
        stylistic_pts += 5
    # NEW: accent-stripping typography tell — SMALL low-weight branch (pt-BR).
    # A file emitted without ç ã õ á é í ó ú â ê reads as broken Portuguese; the
...
# Portuguese tricolon: "X, Y e Z" / "X, Y, e Z". The conjunction is always `e`
# in Portuguese (no `y`/`e` allomorphy as in Spanish). Matches the
# comma-comma-conjunction rhythm the same way the master EN/FR detectors do.
TRICOLON_PT = re.compile(r"\b\w+,\s*\w+,?\s*e\s+\w+\b", flags=re.IGNORECASE)


def detect_tricolons(text: str) -> int:
    return len(TRICOLON_PT.findall(text))


# The accented characters native Portuguese prose carries: the cedilla, the
# tildes (ã õ), and the acute/grave/circumflex/diaeresis vowels, both cases.
# These are the surface that disappears when AI/plain-text output strips
# accents ("informação" → "informacao", "você" → "voce", "não" → "nao").
_ACCENTED_PT_RX = re.compile(r"[áàâãéêíóôõúüçÁÀÂÃÉÊÍÓÔÕÚÜÇ]")
# Count *letters* as the denominator (Unicode word chars minus digits/underscore
# would be noisy; \w includes accented letters so they are in the denominator).
...
# in Portuguese (native ratio ≈ 0.03–0.08), so a short snippet can legitimately
...
def detect_accent_stripping(text: str, threshold: float) -> dict:
    """Portuguese typography tell (ported from the master French accent
    hard-check): an entire file emitted with its accents stripped reads as
    broken Portuguese on sight.

    We compute the ratio of accented Portuguese characters
    (`[áàâãéêíóôõúüçÁÀÂÃÉÊÍÓÔÕÚÜÇ]`) to total letters. Native pt-BR prose sits
    around 0.03–0.08; a value far below the floor `threshold` means the accents
    were dropped. The detector flags only when the ratio is below the floor AND
    the text is long enough (≥ ACCENT_MIN_WORDS words) for the ratio to be
    meaningful — guarding against false positives on short, legitimately
    ...
    accented = len(_ACCENTED_PT_RX.findall(text))
...
def render_human(result: dict) -> str:
    lines: list[str] = []
    lines.append("=" * 60)
    lines.append(f"  human-writer-pt / analyze.py — {result.get('input_file', '<stdin>')}")
    lines.append("=" * 60)
    lines.append(
...
def main() -> int:
    p = argparse.ArgumentParser(description="Deterministic AI-tell analyzer (Brazilian Portuguese)")
    p.add_argument("--input", type=Path, help="Input file (markdown/txt). Stdin if omitted.")
    p.add_argument("--lang", choices=["pt"], required=True)
